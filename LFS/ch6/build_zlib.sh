...
mv -v /usr/lib/libz.so.* /lib 2>&1 | tee ${LOG_DIR}/${FILE_NAME}-make-install.log
ln -sfv ../../lib/$(readlink /usr/lib/libz.so) /usr/lib/libz.so 2>&1 | tee ${LOG_DIR}/${FILE_NAME}-make-install.log
